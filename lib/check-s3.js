...
(function(){
	...
	module.exports = function(app){
		...
		function headBucket(){
			...
			s3.headBucket(params, function(err, data) {
				if(err && err.code != 'NotFound'){
					...
					config.aws_s3_domain = 'https://' + config.settings.aws_bucket + '.s3.amazonaws.com';
		...
		function createBucket(){
			...
			var params = {
				Bucket: config.settings.aws_bucket, /* required */
				ACL: 'public-read',
			...
			s3.createBucket(params, function(err, data) {
				if(err){
					...
					config.settings.aws_s3_domain = 'https://' + config.settings.aws_bucket + '.s3.amazonaws.com';
