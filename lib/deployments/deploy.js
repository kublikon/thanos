		...
		start: function(options, cb){
			...
			function describeInstanceStatus(id){
				...
				ec2.describeInstanceStatus(params, function(err, data) {
					...
					} else if(data.InstanceStatuses.length > 0){
			...
			function createCodeDeploy(){
				...
				codeDeploy.createDeployment(params, function(err, data) {
					if (err){
						...
						save(undefined, 'create code deploy: Deployment in progress.');
			...
			function getDeployment(id){
				...
				codeDeploy.getDeployment(params, function(err, data) {
					...
					} else if(data.deploymentInfo.status == 'Succeeded' || data.deploymentInfo.status == 'Failed' || data.deploymentInfo.status == 'Stopped'){
						save(undefined, 'deployment: ' + data.deploymentInfo.status, data.deploymentInfo.status);
					} else {
						setTimeout(function(){
							getDeployment(id);
						}, 500);
			...
			// save progress
			function save(error, message, status){
