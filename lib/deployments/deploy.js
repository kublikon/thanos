		...
		start: function(app, options, cb){
			...
			function createLaunchConfiguration(){
				...
				var params = {
					...
					BlockDeviceMappings: [
						{
							DeviceName: options.device_name, /* required */
							Ebs: {
								DeleteOnTermination: settings.delete_on_termination,
								// Encrypted: false,
								// Iops: 0,
				...
				if(options.user_data){
					params.UserData = new Buffer(options.user_data).toString('base64');
				} 
			...
			function createLoadBalancer(data){
				...
				var params = {
					Listeners: [ /* required */
						...
						// 	LoadBalancerPort: 443, /* required */
						// 	Protocol: 'SSL', /* required */
						// 	InstanceProtocol: 'TCP',
						// 	// SSLCertificateId: 'STRING_VALUE'
						// },
						/* more items */
				...
				app.elb.createLoadBalancer(params, function(err, data) {
					if (err){
						...
						configureHealthCheck(data);
					}
				});
			};

			function configureHealthCheck(data){

				var params = {
					HealthCheck: { /* required */
						HealthyThreshold: 5, /* required */
						Interval: 30, /* required */
						Target: 'HTTP:80/ping', /* required */
						Timeout: 5, /* required */
				...
				app.elb.configureHealthCheck(params, function(err, data) {
					if (err){
						return cb('configure health check: ' + err);
					} else {
			...
			function createAutoScalingGroup(data){
				...
				var params = {
					...
					MaxSize: options.max_size, /* required */
					MinSize: options.min_size, /* required */
					AvailabilityZones: [
						'us-east-1a'
					],
					// DefaultCooldown: 0,
