	...
	module.exports = function(app){
		...
		app.get('/api/deployments', function(req, res){
			...
			function getIAMRoles(cb){
				deploy.getIAMRoles(app, {}, function(err, data){
					if(err){
						cb('Unable to get deployments');
					} else {
						res_data.roles = data;
			...
			function getAMIs(cb){
				deploy.getAMIs(app, {}, function(err, data){
					if(err){
						cb('Unable to get deployments');
					} else {
						res_data.AMIs = data;
			...
			function getGroups(cb){
				deploy.getGroups(app, {}, function(err, data){
					if(err){
						cb('Unable to get deployments');
					} else {
			...
			function listApplications(cb){
				deploy.listApplications(app, {}, function(err, data){
					if(err){
						cb('Unable to get deployments');
					} else {
		...
		app.get('/api/deployments/compare', function(req, res){
	    	...
	    	function getCompareOne(cb){
	    		if(req.query.compare_1 != ''){
					deploy.getAll(app, req.query.compare_1, function(err, data){
						if(err){
							cb(err);
						} else {
			...
			function getCompareTwo(cb){
				if(req.query.compare_2 != ''){
					deploy.getAll(app, req.query.compare_2, function(err, data){
						if(err){
							cb(err);
						} else {
	    ...
	    app.post('/api/deployment', function(req, res){
	    	...
	    	security.auth(req, res, db, 'developer', function(){
		    	...
		    	deploy.start(app, req.body, function(err, data){
					if(err){
						saveDeployment(err);
					} else {
		...
		app.post('/api/deployment/redeploy', function(req, res){
	    	...
	    	security.auth(req, res, db, 'developer', function(){
		    	...
		    	deploy.redeploy(app, req.body, function(err, data){
					if(err){
						saveDeployment(err);
					} else {
						saveDeployment(undefined, data);
	    ...
	    app.post('/api/deployment/stop', function(req, res){
	    	...
	    	security.auth(req, res, db, 'developer', function(){
	    		deploy.stop(app, req.body, function(err, data){
	    			if(err){
	    ...
	    app.post('/api/deployment/remove', function(req, res){
	    	...
	    	security.auth(req, res, db, 'developer', function(){
	    		deploy.remove(app, req.body, function(err, data){
	    			if(err){
	    				res.end(check.error(err, 'deployment', 107));
	    			} else {
