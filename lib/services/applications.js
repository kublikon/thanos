	...
	module.exports = function(app){
		...
		app.get('/api/applications', function(req, res){
			...
			security.auth(req, res, db, 'all', function(){
		    	...
		    	deploy.listApplications(app, {}, function(err, data){
		    		if(err){
						res.end(check.error(err, 'applications', 107));
					} else {
		...
		app.post('/api/application/update', function(req, res){
			...
			security.auth(req, res, db, 'developer', function(){
		    	...
		    	deploy.updateApplication(app, req.body, function(err, data){
		    		if(err){
		...
		app.post('/api/application/remove', function(req, res){
			...
			security.auth(req, res, db, 'developer', function(){
		    	...
		    	deploy.removeApplication(app, req.body, function(err, data){
		    		if(err){
		...
		app.post('/api/application', function(req, res){
			...
			security.auth(req, res, db, function(){
		    	...
		    	deploy.createApplication(app, req.body, function(err, data){
		    		if(err){
