/*
-- Service for user:
-- get users, get, create, update, delete
*/
	...
	module.exports = function(app){
	    ...
	    app.post('/api/user/update', function(req, res){
	    	...
	    	security.auth(req, res, db, 'admin', function(){
		    	...
		    	function updateAuth(){
		    		...
		    		var update = {
		    			account_type: req.body.user.auth.account_type
		    		}

		    		db.auth.update({ _id: req.body.user.auth }, { $set: update }, { multi: false }, function(err){
		    			if(err){
		    				res.end(check.error('Unable to update auth', 'user', 237));
		    			} else {
		    				if(req.body.profile_m){
			    				res.end(check.validate({ profile: config.settings.aws_s3_domain + '/' + profile_m }, 'user'));
			    			} else {
		...
		app.post('/api/user/delete', function(req, res){
	    	...
	    	security.auth(req, res, db, 'admin', function(){
	    		...
	    		.exec(function(err, data){
	    			if(err){
	    				res.end(check.error('Unable to delete account', 'auth', 105));
	    			} else {
	    				db.auth.remove({ _id: data.auth }, function(err){
	    					if(err){
	    						res.end(check.error('Unable to delete account', 'auth', 105));
	    					} else {
	    						deleteSession();
	    				...
	    				function deleteSession(){
		    				db.session.remove({ user: req.body.id }, function(err){
		    					if(err){
		    						res.end(check.error('Unable to delete account', 'auth', 105));
		    					} else {
		    						data.remove(function(err){
		    							if(err){
		    								...
		    								res.end(check.log('deleted', 'auth'));
		    							}
		    						});
		    					}
		    				});
