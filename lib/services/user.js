	...
	module.exports = function(app){
				...
				if(!req.params.id){
					...
					db.user.findOne({ _id: req.params.id }, function(err, userData){
						if(err){
							...
							res.end(check.validate(userData, 'user'));
						}
					});
				}
			});
	    ...
	    app.post('/api/user', function(req, res){
	    	...
	    	function saveUser(auth_id){
		    	var profile_s = (req.body.profile_s)? req.body.profile_s : config.default_profile_s,
		    		profile_m = (req.body.profile_m)? req.body.profile_m : config.default_profile_m,
		    		profile_l = (req.body.profile_l)? req.body.profile_l : config.default_profile_l;
